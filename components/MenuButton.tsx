...
import { ClipboardListIcon, NewspaperIcon, UsersIcon } from '@heroicons/react/outline';
...
const MenuButton: React.FC<MenuButtonProps> = ({ name, subText, noti, icon }) => {
  return (
    <div className="relative ease-in duration-100 h-full px-6 py-2 border-black border-x-2 border-t-2 border-b-4 rounded-lg overflow-hidden select-none cursor-pointer hover:bg-gray-50 hover:text-gray-900 flex items-center">
      ...
      <div className="ml-auto flex-shrink-0 text-black">
        ...
        {icon === 'connect' && <UsersIcon className="h-10 w-10 md:h-14 md:w-14" aria-hidden="true" />}
      </div>
      {noti && (
        <button
          type="button"
